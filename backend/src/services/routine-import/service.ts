...
export function matchRoutine(routine: ImportedRoutine, catalog: ExerciseDTO[]): RoutineImportResult {
  ...
  const days = routine.days.map(day => {
    ...
    for (const id of groups) {
      ...
      if (members.length === 2 && first && second && !members.some(member => member.exercise.match.exercise?.type === 'CARDIO') && second.index === first.index + 1 && first.exercise.sets !== null && first.exercise.restSeconds !== null && first.exercise.sets === second.exercise.sets && first.exercise.restSeconds === second.exercise.restSeconds) continue
